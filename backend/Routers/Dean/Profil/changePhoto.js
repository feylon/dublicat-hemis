...
const upload = multer({
  ...
  router.post("/",  [checkToken, upload.single('image')], async (req, res)=>{
    ...
    try {
    let fileurl = await global.pool.query("Select profil_url from  dean where id = $1", [adminId]);
    ...
    try {
        await global.pool.query (`
            update dean set profil_url = $1 where id = $2;`, ['profil_pictures/' + req.body.file.url, adminId]);
